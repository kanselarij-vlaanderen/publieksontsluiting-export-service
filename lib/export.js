...
async function exportNewsItems(uuid, sessionUri, tmpGraph, exportFileBase, agendaUri, exportGraphNewsItems) {
  ...
  let procedurestapResults = [];
  for (let agendaItem of agendaItems) {
    await copyAgendaItemDetails(agendaItem.uri, tmpGraph);
    await insertAgendaItemDetails(agendaItem.uri, exportGraphNewsItems);
    await copyNewsItemForAgendaItemTreatment(agendaItem.behandeling, sessionUri,  exportGraphNewsItems, 'nieuws');
    ...
    if (agendaItem.procedurestap) { // Copy mandatee info etc
      ...
      await insertProcedurestapModelCompat(agendaItem.procedurestap, exportGraphNewsItems);
      const procedurestap = await getProcedurestap(tmpGraph, agendaItem.procedurestap);
      procedurestapResults = procedurestapResults.concat(procedurestap);
    }
  }

  const mandatees = uniq(procedurestapResults.map(p => p.mandatee).filter(m => m != null));
...
async function exportAnnouncements(uuid, sessionUri, tmpGraph, exportFileBase, agendaUri, exportGraphMededelingen) {
  ...
  for (let mededeling of mededelingen) {
    await copyAgendaItemDetails(mededeling.uri, tmpGraph);
    await insertAgendaItemDetails(mededeling.uri, exportGraphMededelingen);
    await copyNewsItemForAgendaItemTreatment(mededeling.behandeling, sessionUri,  exportGraphMededelingen, 'mededeling');
    await copyDocumentsForAgendapunt(mededeling.uri, tmpGraph);
    if (mededeling.procedurestap) {
      ...
      await insertProcedurestapModelCompat(mededeling.procedurestap, exportGraphMededelingen);
      const procedurestap = await getProcedurestap(tmpGraph, mededeling.procedurestap);
      procedurestapResults = procedurestapResults.concat(procedurestap); 
    }
  }
  const mandatees = uniq(procedurestapResults.map(p => p.mandatee).filter(m => m != null));
  ...
  await calculatePriorityMededelingen(exportGraphMededelingen, tmpGraph);
