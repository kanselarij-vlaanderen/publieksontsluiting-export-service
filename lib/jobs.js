...
async function executeJob(uuid) {
  ...
  try {
    ...
    const files = await generateExport(job);
    await updateJob(uuid, FINISHED);
    await createTtlToDeltaTask(files);
...
async function createTtlToDeltaTask(files) {
  ...
  const taskUuid = uuid();
  const taskUri = `<http://mu.semte.ch/graphs/public/delta-task/${taskUuid}>`; // TODO fix URI

  const fileStatements = files.map((file) => {
    const fileUuid = uuid();
    const fileUri = `<http://mu.semte.ch/graphs/public/file/${fileUuid}>`; // TODO fix URI
  ...
  // TODO make sure to insert data via mu-authorization
  await query(`
