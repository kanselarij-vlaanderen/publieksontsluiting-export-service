...
async function createJob(jobUuid, session, scope, documentNotification) {
  const jobUri = `http://valvas.data.gift/public-export-jobs/${jobUuid}`;
...
async function addGraphAndFileToJob(uuid, graph, file) {
  ...
  let fileStatement = '';
  if (graph) {
    graphStatement = `?job ext:graph ${sparqlEscapeUri(graph)} . `;
    fileStatement = `?job ext:file ${sparqlEscapeUri(file)} . ${sparqlEscapeUri(file)} dct:created ${sparqlEscapeDateTime(new Date())} .`;
  }
...
async function createTtlToDeltaTask(job) {
  ...
  if (result.results.bindings.length) {
    ...
    const fileStatements = result.results.bindings.map((binding) => {
      ...
      const physicalFileUri = `<${binding['file'].value}>`;
      return `
